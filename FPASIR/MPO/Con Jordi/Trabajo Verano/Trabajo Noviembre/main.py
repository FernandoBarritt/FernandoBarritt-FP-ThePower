...
def crear_archivo():
    while True:
        archivo = input("Nombra el archivo que quieres crear:").strip()
        if archivo:
            try:
               open("archivo", "x")
            ...
            #Errores de archivo ya existente o errores de sistema por incopatibilidad o permisos
                ...
                print(f"Error: El directorio {archivo} ya existe. Prueba otro nombre")
            except OSError as e:
                print(f"Error del Sistema: \n{e}")
        else:
...
def escribir_archivo():
    while True:
        archivo_a_editar = (input("¿Qué archivo quieres editar?: ")).strip()

        if archivo_a_editar:
            try:
                os.path.abspath(archivo_a_editar)
                break
            except FileNotFoundError:
                print(f"No existe el archivo {archivo_a_editar}. Intentelo otra vez")
            except OSError as f:
                print(f"Error del Sistema: \n{f}")
        else:
